 ...
 *
 * MODIFICATIONS FROM https://github.com/mjansson/mdns/releases/tag/1.4.3:
 *  - Added check that an mDNS packet with a question in it is actually a query
 *    instead of a unicast response. Unicast queries from localhost were causing
 *    the server to respond to itself indefinitely, basically DDoSing itself.
 *  - Application of clang-format (automated in this repo).
 */
...
//! user level ephemeral port. To run discovery service listening for incoming
//! discoveries and queries, you must set MDNS_PORT as port.
static inline int mdns_socket_setup_ipv4(
    int sock, const struct sockaddr_in *saddr
);
...
//! user level ephemeral port. To run discovery service listening for incoming
//! discoveries and queries, you must set MDNS_PORT as port.
static inline int mdns_socket_setup_ipv6(
    int sock, const struct sockaddr_in6 *saddr
);
...
//! parsed.
static inline size_t mdns_socket_listen(
    int sock,
    void *buffer,
    size_t capacity,
...
static inline size_t mdns_discovery_recv(
    int sock,
    void *buffer,
    size_t capacity,
    mdns_record_callback_fn callback,
...
static inline int mdns_query_send(
    int sock,
    mdns_record_type_t type,
    const char *name,
    size_t length,
...
static inline int mdns_multiquery_send(
    int sock,
    const mdns_query_t *query,
    size_t count,
    void *buffer,
    size_t capacity,
    uint16_t query_id
...
//! Receive unicast responses to a mDNS query sent with mdns_discovery_recv,
...
static inline size_t mdns_query_recv(
    int sock,
    void *buffer,
    size_t capacity,
    mdns_record_callback_fn callback,
...
static inline int mdns_announce_multicast(
    int sock,
    void *buffer,
    size_t capacity,
    mdns_record_t answer,
...
static inline int mdns_goodbye_multicast(
    int sock,
    void *buffer,
    size_t capacity,
    mdns_record_t answer,
...
static inline mdns_string_t mdns_record_parse_ptr(
    const void *buffer,
    size_t size,
    size_t offset,
    size_t length,
...
//! record
static inline mdns_record_srv_t mdns_record_parse_srv(
    const void *buffer,
    size_t size,
    size_t offset,
    size_t length,
...
static inline struct sockaddr_in *mdns_record_parse_a(
    const void *buffer,
    size_t size,
    size_t offset,
    size_t length,
...
static inline struct sockaddr_in6 *mdns_record_parse_aaaa(
    const void *buffer,
    size_t size,
    size_t offset,
    size_t length,
...
static inline size_t mdns_record_parse_txt(
    const void *buffer,
    size_t size,
    size_t offset,
    size_t length,
...
static inline mdns_string_t mdns_string_extract(
    const void *buffer, size_t size, size_t *offset, char *str, size_t capacity
);

static inline int mdns_string_skip(
    const void *buffer, size_t size, size_t *offset
);

static inline size_t mdns_string_find(
    const char *str, size_t length, char c, size_t offset
);
...
//! updated.
static inline int mdns_string_equal(
...
static inline void *mdns_string_make(
    void *buffer,
    size_t capacity,
    void *data,
    const char *name,
...
static inline size_t mdns_string_table_find(
    mdns_string_table_t *string_table,
    const void *buffer,
    size_t capacity,
    const char *str,
...
static inline int mdns_socket_setup_ipv4(
...
#endif
    setsockopt(
        sock, IPPROTO_IP, IP_MULTICAST_TTL, (const char *)&ttl, sizeof(ttl)
    );
    setsockopt(
        sock, IPPROTO_IP, IP_MULTICAST_LOOP, (const char *)&loopback,
        sizeof(loopback)
    );
    ...
    if (setsockopt(
            sock, IPPROTO_IP, IP_ADD_MEMBERSHIP, (char *)&req, sizeof(req)
        ))
        return -1;
...
static inline int mdns_socket_setup_ipv6(
...
#endif
    setsockopt(
        sock, IPPROTO_IPV6, IPV6_MULTICAST_HOPS, (const char *)&hops,
        sizeof(hops)
    );
    setsockopt(
        sock, IPPROTO_IPV6, IPV6_MULTICAST_LOOP, (const char *)&loopback,
        sizeof(loopback)
    );
    ...
    if (setsockopt(
            sock, IPPROTO_IPV6, IPV6_JOIN_GROUP, (char *)&req, sizeof(req)
        ))
        return -1;
    ...
    if (!saddr) {
        ...
        setsockopt(
            sock, IPPROTO_IPV6, IPV6_MULTICAST_IF, (const char *)&ifindex,
            sizeof(ifindex)
        );
#ifndef _WIN32
        sock_addr.sin6_addr = in6addr_any;
...
static inline mdns_string_pair_t mdns_get_next_substring(
    const void *rawdata, size_t size, size_t offset
) {
...
static inline int mdns_string_equal(
    ...
    do {
        ...
        if (strncasecmp(
                (const char *)
                    MDNS_POINTER_OFFSET_CONST(buffer_rhs, rhs_substr.offset),
                (const char *)
                    MDNS_POINTER_OFFSET_CONST(buffer_lhs, lhs_substr.offset),
                rhs_substr.length
            ))
            return 0;
...
static inline size_t mdns_string_table_find(
    ...
    if (!string_table) return MDNS_INVALID_POS;

    for (size_t istr = 0; istr < string_table->count; ++istr) {
        if (string_table->offset[istr] >= capacity) continue;
        size_t offset = 0;
        mdns_string_pair_t sub_string = mdns_get_next_substring(
            buffer, capacity, string_table->offset[istr]
        );
        if (!sub_string.length || (sub_string.length != first_length)) continue;
        if (memcmp(
                str, MDNS_POINTER_OFFSET(buffer, sub_string.offset),
                sub_string.length
            ))
            continue;
        ...
        while (offset < total_length) {
            ...
            if (dot_pos == MDNS_INVALID_POS) dot_pos = total_length;
            ...
            if (memcmp(
                    str + offset,
                    MDNS_POINTER_OFFSET(buffer, sub_string.offset),
                    sub_string.length
                ))
                break;
        ...
        if (offset >= total_length) return string_table->offset[istr];
    }

    return MDNS_INVALID_POS;
}

static inline void mdns_string_table_add(
    mdns_string_table_t *string_table, size_t offset
) {
    ...
    if (++string_table->next >= table_capacity) string_table->next = 0;
}
...
static inline size_t mdns_string_find(
    ...
    if (found) return (size_t)MDNS_POINTER_DIFF(found, str);
    return MDNS_INVALID_POS;
}

static inline void *mdns_string_make_ref(
    void *data, size_t capacity, size_t ref_offset
) {
...
static inline void *mdns_string_make(
    ...
    while (last_pos < length) {
        ...
        size_t sub_length =
            ((pos != MDNS_INVALID_POS) ? pos : length) - last_pos;
        ...
        if (remain <= (sub_length + 1)) return 0;
...
static inline size_t mdns_records_parse(
    int sock,
    const struct sockaddr *from,
    size_t addrlen,
    const void *buffer,
    ...
    for (size_t i = 0; i < records; ++i) {
        ...
        if (((*offset) + 10) > size) return parsed;
        ...
        *offset += 10;
        ...
        if (length <= (size - (*offset))) {
            ...
            if (callback && callback(
                                sock, from, addrlen, type, query_id, rtype,
...
static inline int mdns_unicast_send(
    int sock,
    const void *address,
    size_t address_size,
    const void *buffer,
    ...
    if (sendto(
            sock, (const char *)buffer, (mdns_size_t)size, 0,
...
static inline int mdns_multicast_send(
    int sock, const void *buffer, size_t size
) {
    ...
    if (sendto(
            sock, (const char *)buffer, (mdns_size_t)size, 0, saddr, saddrlen
        ) < 0)
        return -1;
    return 0;
}
...
static inline int mdns_discovery_send(int sock) {
    return mdns_multicast_send(
        sock, mdns_services_query, sizeof(mdns_services_query)
    );
}
...
static inline size_t mdns_discovery_recv(
...
#endif
    mdns_ssize_t ret = recvfrom(
        sock, (char *)buffer, (mdns_size_t)capacity, 0, saddr, &addrlen
    );
    if (ret <= 0) return 0;
    ...
    // According to RFC 6762 the query ID MUST match the sent query ID (which is
    // 0 in our case)
    ...
    for (i = 0; i < questions; ++i) {
        ...
        if (!mdns_string_equal(
                buffer, data_size, &offset, mdns_services_query,
    ...
    for (i = 0; i < answer_rrs; ++i) {
        ...
        if (!is_answer && !mdns_string_skip(buffer, data_size, &offset)) break;
        size_t name_length = offset - name_offset;
        if ((offset + 10) > data_size) return records;
        data = (const uint16_t *)MDNS_POINTER_OFFSET(buffer, offset);
        ...
        data += 2;
        uint16_t length = mdns_ntohs(data++);
        if (length > (data_size - offset)) return 0;
        ...
        if (is_answer) {
            ...
            if (callback &&
                callback(
                    sock, saddr, addrlen, MDNS_ENTRYTYPE_ANSWER, query_id,
                    rtype, rclass, ttl, buffer, data_size, name_offset,
                    name_length, offset, length, user_data
                ))
                return records;
        }
    ...
    records = mdns_records_parse(
        sock, saddr, addrlen, buffer, data_size, &offset,
        MDNS_ENTRYTYPE_AUTHORITY, query_id, authority_rrs, callback, user_data
    );
    total_records += records;
    if (records != authority_rrs) return total_records;

    records = mdns_records_parse(
        sock, saddr, addrlen, buffer, data_size, &offset,
        MDNS_ENTRYTYPE_ADDITIONAL, query_id, additional_rrs, callback, user_data
    );
    total_records += records;
    if (records != additional_rrs) return total_records;
...
static inline size_t mdns_socket_listen(
    ...
    if (!(
            flags & 0x8000
        )) { // ADDED BY JEROEN VAN STRATEN: don't respond to yourself, silly.
        for (int iquestion = 0; iquestion < questions; ++iquestion) {
            ...
            if (dns_sd && flags) continue;

            ++total_records;
            if (callback &&
                callback(
                    sock, saddr, addrlen, MDNS_ENTRYTYPE_QUESTION, query_id,
                    rtype, rclass, 0, buffer, data_size, question_offset,
    ...
    size_t offset = MDNS_POINTER_DIFF(data, buffer);
    records = mdns_records_parse(
        sock, saddr, addrlen, buffer, data_size, &offset, MDNS_ENTRYTYPE_ANSWER,
        query_id, answer_rrs, callback, user_data
    );
    total_records += records;
    if (records != answer_rrs) return total_records;
...
static inline int mdns_multiquery_send(
    ...
    for (size_t iq = 0; iq < count; ++iq) {
        // Name string
        data = mdns_string_make(
            buffer, capacity, data, query[iq].name, query[iq].length, 0
        );
        if (!data) return -1;
        // Record type
...
static inline void *mdns_answer_add_question_unicast(
    ...
    data = mdns_string_make(
        buffer, capacity, data, name, name_length, string_table
    );
    if (!data) return 0;
    size_t remain = capacity - MDNS_POINTER_DIFF(data, buffer);
...
static inline void *mdns_answer_add_record_header(
    void *buffer,
    size_t capacity,
    void *data,
    ...
    data = mdns_string_make(
        buffer, capacity, data, record.name.str, record.name.length,
        string_table
    );
    if (!data) return 0;
    size_t remain = capacity - MDNS_POINTER_DIFF(data, buffer);
...
static inline void *mdns_answer_add_record(
    void *buffer,
    size_t capacity,
    void *data,
    mdns_record_t record,
    ...
    if (!data || (record.type == MDNS_RECORDTYPE_TXT)) return data;

    data = mdns_answer_add_record_header(
        buffer, capacity, data, record, string_table
    );
    if (!data) return 0;
    ...
    switch (record.type) {
        case MDNS_RECORDTYPE_PTR:
            data = mdns_string_make(
                buffer, capacity, data, record.data.ptr.name.str,
        ...
        case MDNS_RECORDTYPE_SRV:
            ...
            data = mdns_string_make(
                buffer, capacity, data, record.data.srv.name.str,
...
static inline void mdns_record_update_rclass_ttl(
    mdns_record_t *record, uint16_t rclass, uint32_t ttl
) {
    if (!record->rclass) record->rclass = rclass;
    if (!record->ttl || !ttl) record->ttl = ttl;
...
static inline void *mdns_answer_add_txt_record(
    void *buffer,
    size_t capacity,
    void *data,
    ...
    for (size_t irec = 0; data && (irec < record_count); ++irec) {
        if (records[irec].type != MDNS_RECORDTYPE_TXT) continue;
        ...
        // TXT strings are unlikely to be shared, just make then raw. Also need
        // one byte for termination, thus the <= check
        size_t string_length =
            record.data.txt.key.length + record.data.txt.value.length + 1;
        if (!data) return 0;
        remain = capacity - MDNS_POINTER_DIFF(data, buffer);
        if ((remain <= string_length) || (string_length > 0x3FFF)) return 0;
        ...
        *strdata++ = '=';
        memcpy(
            strdata, record.data.txt.value.str, record.data.txt.value.length
        );
    ...
    if (record_data)
        mdns_htons(
            record_length, (uint16_t)MDNS_POINTER_DIFF(data, record_data)
        );

    return data;
}

static inline uint16_t mdns_answer_get_record_count(
...
static inline int mdns_query_answer_unicast(
    ...
    if (capacity < (sizeof(struct mdns_header_t) + 32 + 4)) return -1;
    ...
    // Fill in question
    data = mdns_answer_add_question_unicast(
        buffer, capacity, data, record_type, name, name_length, &string_table
    );
    ...
    answer.ttl = ttl;
    data =
        mdns_answer_add_record(buffer, capacity, data, answer, &string_table);
    ...
    for (size_t irec = 0; data && (irec < authority_count); ++irec) {
        ...
        if (!record.ttl) record.ttl = ttl;
        data = mdns_answer_add_record(
            buffer, capacity, data, record, &string_table
        );
    }
    data = mdns_answer_add_txt_record(
        buffer, capacity, data, authority, authority_count, rclass, ttl,
        &string_table
    );
    ...
    data = mdns_answer_add_txt_record(
        buffer, capacity, data, additional, additional_count, rclass, ttl,
        &string_table
    );
    if (!data) return -1;
...
static inline mdns_record_srv_t mdns_record_parse_srv(
    ...
    if ((size >= offset + length) && (length >= 8)) {
        ...
        offset += 6;
        srv.name =
            mdns_string_extract(buffer, size, &offset, strbuffer, capacity);
    }
    return srv;
}
