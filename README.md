...
to worry about game internals or low-level interprocess communication. So
**if you're not a developer, you should probably stop reading here.**
...
to people who are not so tech-savvy. For that reason, TruckTel includes a tiny
mDNS server as well, through which it announces itself as being
`trucktel.local`. Most operating systems nowadays support mDNS resolutions for
the `.local` domain, so, router, firewall, and LAN party shenanigans aside,
`trucktel.local` should resolve to the correct IP address once the game is
running. Users still need to know the port though; while mDNS allows for ports
to be announced (and TruckTel does this) browsers always just default to
port 80. You could probably resolve the port number in a mobile app, though.
